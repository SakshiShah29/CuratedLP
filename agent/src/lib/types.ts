...
export interface BudgetStatus {
  ...
  canSpend: boolean;
  /**
   * FULL     >$1.00 remaining → uniswap + olas + venice
   * PARTIAL  $0.10–$1.00     → uniswap + venice (skip olas, use cache)
   * MINIMAL  <$0.10          → uniswap + venice (free sources only)
   * CACHE_ONLY $0.00         → pool-reader + cache + venice (partial)
   */
...
// ─── Uniswap Trading API (from uniswap-data.ts — Person B) ─────────────────
...
// ─── Venice / EigenCompute (from venice-analyze.ts, eigencompute.ts — Person B)
